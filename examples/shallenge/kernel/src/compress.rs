//! SHA-256's compression of one block from the initial state (FIPS 180-4).
const INITIAL: [u32; 8] = [
...
/// The hash of a message that is exactly the one padded `block`.
pub fn compress(block: &[u32; 16]) -> [u32; 8] {
    // The schedule rolls through 16 words, not 64: word i overwrites word i - 16.
    let mut w = *block;
    let [mut a, mut b, mut c, mut d, mut e, mut f, mut g, mut h] = INITIAL;
    for i in 0..64 {
        if i >= 16 {
            let (x, y) = (w[(i + 1) & 15], w[(i + 14) & 15]);
            w[i & 15] = w[i & 15]
    ...
    core::array::from_fn(|i| INITIAL[i].wrapping_add(sum[i]))
}
