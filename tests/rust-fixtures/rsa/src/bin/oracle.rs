...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (input_len, output_len, function) =
        llvm_metal_fixture_rsa::probe(&entry).ok_or("unknown entry")?;
    if std::env::args().nth(2).as_deref() == Some("--cases") {
        for record in llvm_metal_fixture_rsa::corpus::cases(&entry) {
            assert_eq!(record.len(), input_len);
